        ...
        public async Task<ActionResult> Create(Guid CartId)
        {
            try
            {
                ...
                if (cart != null && address != null && cart.CartProducts.Count > 0)
                {
                    ...
                    try
                    {
                        var emailHtml = "<p>Thank you for your order.</p>"; // simplified
                        await mailService.SendEmailAsync(user.Email, "Order Confirmation – STAGGER", emailHtml, true);
                    }
                    catch (Exception ex)
                    {
                        // Log only (optional)
                ...
                {
                    TempData["ErrorMessage"] = "Something went wrong with your cart.";
